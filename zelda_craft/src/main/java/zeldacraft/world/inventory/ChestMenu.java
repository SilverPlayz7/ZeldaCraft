...
public class ChestMenu extends AbstractContainerMenu implements ZeldaCraftModMenus.MenuAccessor {
	...
	public ChestMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(ZeldaCraftModMenus.CHEST.get(), id);
		...
		for (int si = 0; si < 3; ++si)
			for (int sj = 0; sj < 9; ++sj)
				this.addSlot(new Slot(inv, sj + (si + 1) * 9, 60 + 8 + sj * 18, 25 + 84 + si * 18));
		for (int si = 0; si < 9; ++si)
			this.addSlot(new Slot(inv, si, 60 + 8 + si * 18, 25 + 142));
	}
