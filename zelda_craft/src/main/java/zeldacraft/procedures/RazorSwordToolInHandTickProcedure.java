...
public class RazorSwordToolInHandTickProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getItem() == ZeldaCraftModItems.RAZOR_SWORD.get()) {
			ZeldaCraftMod.queueServerWork(5, () -> {
				if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == Blocks.AIR.asItem()) {
					if (entity instanceof LivingEntity _entity) {
						ItemStack _setstack = new ItemStack(ZeldaCraftModItems.KOKIRI_SWORD.get()).copy();
