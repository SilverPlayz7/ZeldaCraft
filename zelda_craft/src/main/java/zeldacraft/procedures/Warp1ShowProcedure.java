...
public class Warp1ShowProcedure {
	public static boolean execute(Entity entity) {
		...
		if (!(statueList.size() < 1 + 25 * (entity.getCapability(ZeldaCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ZeldaCraftModVariables.PlayerVariables())).pageNum)) {
			if (!(new Object() {
			...
			}.get(statueList, (int) (int) (0 + 25 * (entity.getCapability(ZeldaCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ZeldaCraftModVariables.PlayerVariables())).pageNum))).isEmpty()) {
				hasWarp = true;
			}
		}
		return hasWarp;
