...
public class Warp1ShowProcedure {
	public static boolean execute(Entity entity) {
		...
		if (!(statueList.size() < 1 + 25 * (entity.getCapability(ZeldaCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElseGet(ZeldaCraftModVariables.PlayerVariables::new)).pageNum)) {
			if (!(new Object() {
			...
			}.get(statueList, (int) (int) (0 + 25 * (entity.getCapability(ZeldaCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElseGet(ZeldaCraftModVariables.PlayerVariables::new)).pageNum))).isEmpty()) {
				hasWarp = true;
			}
		}
		return hasWarp;
