	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (world.getServer() != null) {
			...
			for (ServerLevel worlditerator : world.getServer().getAllLevels()) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(ResourceLocation.parse("zelda_craft:ocarina_songofstorms")), SoundSource.RECORDS, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(ResourceLocation.parse("zelda_craft:ocarina_songofstorms")), SoundSource.RECORDS, 1, 1, false);
					}
				}
