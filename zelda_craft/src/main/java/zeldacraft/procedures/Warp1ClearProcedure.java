...
public class Warp1ClearProcedure {
	public static void execute(Entity entity) {
		...
		if (Screen.hasShiftDown()) {
			...
			if (!(statueList.size() < 0 + 25 * (entity.getCapability(ZeldaCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ZeldaCraftModVariables.PlayerVariables())).pageNum)) {
				for (int index0 = 0; index0 < 5; index0++) {
					{
						statueList.remove((int) (int) (0 + 25 * (entity.getCapability(ZeldaCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ZeldaCraftModVariables.PlayerVariables())).pageNum));
					}
				}
				{
